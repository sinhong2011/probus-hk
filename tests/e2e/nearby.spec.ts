...
const stopLinks = (page: import("@playwright/test").Page) =>
  page.locator('a[href^="/stop/"]');
...
test("falls back to the timetable when every arrival feed is down", async ({ page }) => {
  ...
  await expect(page.locator('[data-eta-state="scheduled"], [data-eta-state="none"]').first()).toBeVisible({
    timeout: 15_000,
  });
});
...
test("offers a retry when the route database cannot be fetched", async ({ page }) => {
  ...
  await expect(page.getByText("未能載入路線資料")).toBeVisible({ timeout: 20_000 });
