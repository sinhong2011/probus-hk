...
describe("saved", () => {
  it("adopts a list saved by an older build, keeping its order", async () => {
    ...
    expect(memory.has("probus:bookmarks")).toBe(true);
  });
  ...
  it("toggles, groups, pins and forgets", async () => {
    ...
    expect(memory.get("probus:bookmarks")).not.toContain('"pinned"');

    saved.toggle(entry("1", "A"));
    await settled();
    expect(saved.has("1", "A")).toBe(false);
    expect(JSON.parse(memory.get("probus:bookmarks") ?? "{}")).not.toHaveProperty("1@A");
  });
